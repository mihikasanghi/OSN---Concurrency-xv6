  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        {
          process_with_highest_priority = p;
        } // Above code block generated by copilot (no prompt given for this one, understood context of code on its own)
        else if (process_with_highest_priority->DP == p->DP)
        {
          if (process_with_highest_priority->scheduled > p->scheduled)
          {
            process_with_highest_priority = p;
          } // Above code block generated by copilot after if condition was specified
    ...
    if (process_with_highest_priority)
    {
      ...
      if (process_with_highest_priority->state == RUNNABLE)
      {
        ...
        c->proc = 0;
        // Above code block partially generated by copilot (wrote first 2 lines only)
      }
